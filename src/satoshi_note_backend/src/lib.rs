...
const CKBTC_CANNISTER_ID : &str = "mxzaz-hqaaa-aaaar-qaada-cai";
...
thread_local! {
    ...
    pub static NOTES_BY_USER: RefCell<BTreeMap<PrincipalName, HashSet<u128>>> = RefCell::new(BTreeMap::new());
    pub static USER_KEYS: RefCell<BTreeMap<Principal, UserStore>> = RefCell::new(BTreeMap::new());
    pub static MARKET : RefCell<BTreeMap<PrincipalName, Vec<u128>>> = RefCell::new(BTreeMap::new());
    pub static SELL_ORDER_STORE : RefCell<BTreeMap<u128, SellOrder>> = RefCell::new(BTreeMap::new());
    pub static NOTES_STORE : RefCell<BTreeMap<u128, EncryptedNote>> = RefCell::new(BTreeMap::new());
}
...
#[update]
fn add_note_to_marketsquare(mut order: SellOrder) {
    let user = caller();
    
    assert!(is_user_registered(user));
    let user_str = user.to_string();
    order.author = user.to_text();
    NOTES_BY_USER.with_borrow_mut(|store| {
        let key = store.get_mut(&user_str).unwrap().remove(&order.note_id);
        if key {
            MARKET.with_borrow_mut(|marketplace| {
               let cont =  marketplace.get_mut(&user_str);
               if cont.is_none() {
                   let mut hs = vec![];
                   hs.push(order.note_id);
                   marketplace.insert(user_str.clone(), hs);
               } else {
                let hs = cont.unwrap();
                hs.push(order.note_id)
               }
            ...
            SELL_ORDER_STORE.with_borrow_mut(|store| {
                store.insert(order.note_id, order)
            });
        }
    })
}
...
fn get_deposit_address() -> String {
    let acc = AccountIdentifier::new(&id(), &Subaccount::from(caller()));
    return acc.to_hex()
}

#[update]
async fn buy_note(note_id: u128) -> Result<(), String> {
    assert!(is_user_registered(caller()));
    let order = SELL_ORDER_STORE.with_borrow(|store| store.get(&note_id).cloned());
    if order.is_none() {
        return Err("Order not Found".to_string());
    }

    let order = order.unwrap();
    let acc = Account{ owner: id(), subaccount: Some(Subaccount::from(Principal::from_text(order.author).unwrap()).0) };
    let arg = TransferArg{ from_subaccount: Some(Subaccount::from(caller()).0), to: acc, fee: None, created_at_time: None, memo: None, amount: order.price };
    let (rslt,) : (Result<Nat, TransferError>,) = call(Principal::from_str(CKBTC_CANNISTER_ID).unwrap(), "icrc1_transfer", (arg,)).await.unwrap();

    if rslt.is_err() {
        return Err("Transfer Error".to_string());
    }

    NOTES_BY_USER.with_borrow_mut(|store| {
        let f = store.get_mut(&caller().to_string());
        if f.is_none() {
...
#[query]
fn get_notes_on_sale() -> Vec<u128> {
    return SELL_ORDER_STORE.with_borrow(|store| store.keys().cloned().collect());
}
...
fn get_notes() -> Vec<EncryptedNote> {
    ...
    assert!(is_user_registered(user));
    let user_str = user.to_string();
    let note_ids = NOTES_BY_USER.with(|notes_ref| {
        notes_ref
            .borrow()
            .get(&user_str)
            .cloned()
    ...
    let mut notes = vec![];
    for id in note_ids {
        let note = NOTES_STORE.with(|store| {
            let c = store.borrow().get(&id).cloned();
            c
...
fn update_note(note: EncryptedNote) {
    ...
    let user_str = user.to_string();
    NOTES_BY_USER.with(|notes_ref| {
        let writer = notes_ref.borrow();
        let c = writer.get(&user_str).unwrap();
        if c.contains(&note.id) {
            NOTES_STORE.with_borrow_mut(|store| {
...
fn add_note(note: String) {
    ...
    NOTES_BY_USER.with(|notes_ref| {
        ...
        user_notes.insert(note_id);
    });

    NOTES_STORE.with_borrow_mut(|store| {
        store.insert(note_id, EncryptedNote { id: note_id, encrypted_text: note })
    });
